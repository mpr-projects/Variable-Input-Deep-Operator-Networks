...
def setup(parent):
    ...
    n_x_points_save = settings['n_x_points_save']  # in total
    ...
    parent.writer.add_data('output_coords', txy_values, 'outputs')
    parent.writer.add_meta('n_output_coords', n_t_points * n_x_points_save)
...
def process_output(parent):
    output_indices = parent.state['output_indices']
    return {f"output_{parent.sid}": parent.output.reshape(
        parent.settings['n_t_points'], -1, 1)[:, output_indices]}
